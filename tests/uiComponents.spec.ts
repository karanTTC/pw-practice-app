...
test('dialog box', async ({ page }) => {
    ...
    await expect(page.getByRole('table').locator('tr', {hasText: "mdo@gmail.com"})).toContainText('mdo@gmail.com')
...
test('web tables 1', async ({ page }) => {
    ...
    const targetAge = targetRow.getByPlaceholder('Age')
    await targetAge.clear()
    await targetAge.fill('25')
...
test('web tables 2', async ({ page }) => {
    ...
    for (const age of ages) {
        ...
        await page.locator('input-filter').getByPlaceholder('Age').fill(age) // wait for 1 second
...
test('date picker 1', async ({ page }) => {
    ...
    await expect(calendarInputField).toHaveValue('Jan 2, 2025')
})
...
test('date picker 2', async ({ page }) => {
    ...
    //const expectedMonthShort = date.getMonth().toString()
